            ...
            <div className="bg-red-50 border border-red-100 rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-semibold text-red-700">
                  {atrasadas.length} obra{atrasadas.length > 1 ? 's' : ''} em atraso
                </span>
              </div>
              <div className="space-y-1.5">
                {atrasadas.map(p => {
                  const dias = p.data_previsao_fim ? diasAtras(p.data_previsao_fim) : 0
                  return (
                    <Link key={p.id} href={`/diario-obra/${p.id}`}
                      className="flex items-center justify-between gap-2 p-2 bg-white/70 rounded-xl hover:bg-white transition-colors">
                      <div className="min-w-0">
                        <p className="text-xs font-medium text-gray-800 truncate">
                          {p.titulo_publico ?? p.proposals?.title ?? 'Sem nome'}
                        </p>
                        {(p.proposals?.leads?.company ?? p.proposals?.leads?.name) && (
            ...
            <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-sm font-semibold text-amber-700">
                  {semRegistro.length} obra{semRegistro.length > 1 ? 's' : ''} sem registro nos últimos 7 dias
                </span>
              </div>
              <div className="space-y-1.5">
                {semRegistro.map(p => {
                  ...
                  return (
                    <Link key={p.id} href={`/diario-obra/${p.id}`}
                      ...
                      <span className="text-xs text-amber-600 shrink-0 font-medium">
                        {ultimoReg ? `último: ${formatDateBR(ultimoReg.data)}` : 'sem registros'}
                      </span>
                    </Link>
                  )
                })}
