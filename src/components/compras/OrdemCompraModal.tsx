...
import { createOrdemCompra, updateOrdemCompra } from '@/app/(crm)/compras/actions'
...
interface InitialOrdem {
  ...
  itens_ordem_compra: { id: string; descricao: string; quantidade: number; unidade: string | null; valor_unitario: number }[]
}
...
interface Props {
  ...
  onClose: () => void
  onSaved: (ordem: any) => void
  initial?: InitialOrdem
}
...
export default function OrdemCompraModal({ fornecedores, projetos, onClose, onSaved, initial }: Props) {
  ...
  const [dataPedido, setDataPedido] = useState(initial?.data_pedido ?? new Date().toISOString().split('T')[0])
  const [dataEntrega, setDataEntrega] = useState(initial?.data_entrega_prevista ?? '')
  const [formaPagamento, setFormaPagamento] = useState(initial?.forma_pagamento ?? '')
  const [observacoes, setObservacoes] = useState(initial?.observacoes ?? '')
  const [itens, setItens] = useState<ItemForm[]>(
    initial?.itens_ordem_compra?.length
      ? initial.itens_ordem_compra.map(i => ({
  ...
  async function handleSave() {
    if (!descricao.trim()) { setError('Informe uma descrição para a ordem'); return }
    const itensValidos = itens.filter(i => i.descricao.trim())
    if (itensValidos.length === 0) { setError('Adicione pelo menos 1 item'); return }
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 sticky top-0 bg-white z-10">
          <h2 className="font-semibold text-gray-900">
            {isEdit ? 'Editar Ordem de Compra' : 'Nova Ordem de Compra'}
          </h2>
        ...
        <div className="p-6 space-y-5">
          {/* Descrição */}
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1.5">Descrição da compra *</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1.5">Obra vinculada</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1.5">Data do pedido</label>
              <input type="date" value={dataPedido} onChange={e => setDataPedido(e.target.value)}
                className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1.5">Entrega prevista</label>
              ...
              {itens.map((item, i) => (
                <div key={i} className="grid grid-cols-12 gap-2 items-start">
                  ...
                  <div className="col-span-2">
                    <input value={item.quantidade} onChange={e => updateItem(i, 'quantidade', e.target.value)}
                      placeholder="Qtd" type="number" min="0" step="any"
                      className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                  <div className="col-span-2">
                    <input value={item.unidade} onChange={e => updateItem(i, 'unidade', e.target.value)}
                      placeholder="un, m²"
                      className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
                  <div className="col-span-2">
                    <input value={item.valor_unitario} onChange={e => updateItem(i, 'valor_unitario', e.target.value)}
                      placeholder="R$ unit."
                      className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
                  </div>
          ...
          <div>
            ...
            <textarea value={observacoes} onChange={e => setObservacoes(e.target.value)}
              rows={2} placeholder="Instruções de entrega, referências, etc."
